...
const sounds = [
  ...
  { id: "ambient", label: "Meditation", icon: Music },
  { id: "piano", label: "Soft Piano", icon: Piano },
...
const BackgroundSoundPicker = () => {
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card/80 p-5 backdrop-blur-sm">
      <p className="mb-3 text-xs font-medium tracking-[0.2em] uppercase text-gold">
        Background Sound
      </p>
        ...
        {sounds.map((sound) => {
          ...
          return (
            <motion.button
              ...
              className={`flex shrink-0 flex-col items-center gap-1.5 rounded-xl px-4 py-3 text-xs font-medium transition-all ${
                isActive
                  ? "gradient-sunrise text-primary-foreground"
                  : "border border-border/50 bg-muted/30 text-muted-foreground hover:border-primary/30"
              }`}
            >
